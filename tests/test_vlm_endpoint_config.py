...
def test_custom_endpoint_never_receives_sync_credential(tmp_path, monkeypatch):
    ...
    def post(url, **kwargs):
        ...
        return httpx.Response(200, json={"choices": [{"message": {"content": "FACTURA NIF B12345678 TOTAL 121,00 EUR Fecha 15/01/2026"}}]}, request=httpx.Request("POST", url))

    monkeypatch.setattr(httpx, "post", post)
    ctx = PageContext(pdf_path=tmp_path / "invoice.pdf", page_index=0,
                      cache=FeatureCache(tmp_path / "state"), pages_dir=tmp_path,
                      page_image_sha="page", config={"config_version": "custom",
                      "vlm_base_url": "https://provider.invalid/v1", "vlm_model": "configured-model",
                      "vlm_server_auth": False, "rungs": {"vlm_local": {"min_field_coverage": 0}}})
