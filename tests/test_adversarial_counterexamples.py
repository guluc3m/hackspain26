...
from filemaid.parse import extractors, normalizers
...
"""
    val_nif, _conf_nif = extractors._nif(markdown_doc)
    assert val_nif == "B12345678"

    val_fecha, _conf_fecha = extractors._fecha(markdown_doc)
    assert val_fecha == "15/02/2026"

    val_pedido, _conf_pedido = extractors._pedido(markdown_doc)
    assert val_pedido == "PO-2026-0099"

    val_base, _conf_base = extractors._base(markdown_doc)
    assert val_base == 1000.0

    val_total, _conf_total = extractors._total(markdown_doc)
...
"""
    val1, _conf1 = extractors._nif(doc_order_1)
...
Cliente: Banco Miralmar S.A. CIF: A58231074
"""
    val2, _conf2 = extractors._nif(doc_order_2)
...
def test_rule_engine_never_pays_on_corrupted_or_inconsistent_extractions():
    ...
    master = MasterData(
        proveedores={
            "B12345678": Proveedor("B12345678", "Proveedor Test", "ES9121000418450200051332")
        },
        pedidos={
            "PO-2026-0001": Pedido(
                "PO-2026-0001", "B12345678", 121.0, estado="PENDIENTE", pagado=False
            )
        },
    )
    ...
    corrupted_fields = {
        ...
        "iban": ExtractionField(
            type="iban", values=[Candidate("regex_iban", "ES9121000418450200051332", 0.9)]
        ),
        ...
        "iva_amount": ExtractionField(
            type="iva_amount", values=[Candidate("regex_iva_amount", 21.0, 0.6)]
        ),
        "fecha": ExtractionField(
            type="fecha", values=[Candidate("regex_fecha", "15/01/2026", 0.7)]
        ),
        "pedido": ExtractionField(
            type="pedido", values=[Candidate("regex_pedido", "PO-2026-0001", 0.7)]
        ),
    }
