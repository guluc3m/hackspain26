...
#adr([Extracción en dos bloques: _features_ y _parser_ — calibrada con el corpus (T10)],
  ...
  evidencia: [Volcado en `video/data_dryrun.json` (T10, dry-run del corpus de 500 PDFs): 471/500 texto usable (94,2 %), 29 a raster, 0 solo-QR, 0 errores; latencias rung 1 media 0,4 ms / p95 2,0 ms y rung 2 media 42,1 ms / p95 73,0 ms; throughput rung 1 medido 2 636 archivos/s. Calibración del rung 3 medida con el corpus (detalle en `video/data_dryrun.json` y `docs/report/escalabilidad_datos.typ`): word-conf 40.0 deja debajo solo los 3 ilegibles (89,7 % pasa), cobertura 0.4 deja el 100 % de las capas de texto por encima. Explotación en la sección Escalabilidad (generada desde el store).],
)
...
#adr([Rung 5 en la nube: deepseek-v4.1-flash; revisión humana no bloqueante],
  ...
  evidencia: [DRILLS MEDIDOS (volcado en `video/data_drills.json`, T12): `rung5-provider-caido` PASS — el proveedor no responde tras 3 intentos y la página entra en cola de revisión con motivo en evidencia, el lote sigue; `backoff-429` PASS — Retry-After respetado, 3 llamadas exactas y delays aplicados según cabecera; `crash-reanudacion` y `ledger-corrupto` PASS (4/4 drills). Cola de revisión en la UI con imagen de página junto a cada lectura candidata y desacuerdo resaltado (T5/T7); números de la corrida real del lote 1 (T14, `video/data_outcomes_lote1.jsonl`): 45 de 500 facturas resuelven ESCALAR y las 45 entran en cola con señal concreta (algún veredicto FAIL o UNKNOWN puntual; 0 casos de evidencia totalmente insuficiente): 5 por `NO_EMBEDDED_INSTRUCTIONS`, 3 por `DATE_VALID_NOT_FUTURE`, 2 por `PEDIDO_EN_REVISION`, 1 por `IBAN_MATCHES_MASTER`, y el resto por desacuerdo de evidencia tras el triage. Decisión registrada en `docs/decisiones/DECISIONS.md` y AGENTS §3 (escalera de extracción).],
)

#adr([Selección de candidato con provenance — colapso determinista y auditoría por candidato (ADR-06)],
  status: "ACEPTADA",
  ...
  decision: [El colapso de candidatos es una pieza propia del motor (`escoger()`, `src/filemaid/rules/escoger.py`) que TODA regla consume vía `ctx.pick_coded(...)`: cada candidato se filtra por tests de formato, se puntúa como confianza × peso del extractor (config `seleccion` por field, con umbral mínimo de puntuación) y se gana a UN único candidato; los empates se resuelven primero por valores idénticos y después por ranking de extractores (el override humano siempre va primero en el desempate). El veredicto no evalúa «todos los candidatos»: cada regla (p. ej. `ORDER_BELONGS_TO_SUPPLIER` cruza pedido + NIF + total contra el maestro, incluido el importe con tolerancia 0,01) decide sobre ESE candidato elegido; la trazabilidad del porqué la da `CandidateAudit`: UN registro por candidato con estado (`ELEGIDO`, `MENOR_PUNTUACION`, `RECHAZADO_FORMATO`, `RECHAZADO_UMBRAL`, `DESVANTAJA_DESEMPATE`), puntuación y motivo. Si ningún candidato supera formato/umbral/confianza, la regla resuelve UNKNOWN (escala), nunca FAIL.],
  consecuencias: [Los falsos NO_PAGAR desaparecen sin tocar extracción ni política: el cambio vive SOLO en el colapso del motor y la corrección del maestro apunta al total correcto; cada decisión registra el candidato elegido y la auditoría completa de los descartados, alimentando Revisión e Impacto; el NO_PAGAR definitivo se reserva para violaciones reales, como exige §6. La puntuación y los pesos son CONFIGURACIÓN versionada por field, coherente con el ADR-01.],
  evidencia: [Volcado en `video/data_lote1.json`: auditoría T17 — 87/108 falsos con la firma exacta (candidato [1409.4, 1705.37], maestro 1705.37); diff medido del reprocesado en `video/data_impacto.json` (motor 1.0.0 → 1.1.0) — 86 NO_PAGAR→PAGAR, 0 regresiones, 1 desviación esperada y documentada (FA-8801 duplicado: su importe ahora cuadra pero sigue NO_PAGAR por `NO_DOUBLE_PAYMENT`, §6); la auditoría por candidato (`CandidateAudit`) está expuesta en la trazabilidad de cada decisión. Números reproducibles con `uv run filemaid emit --out outcomes.jsonl`.],
)
...
Los 8 ADRs anteriores son ADICIONES INCREMENTALES, no una lista cerrada: el jurado puede leer los ADR-01–05 como el núcleo arquitectónico y los ADR-06–08 como extensiones sobre decisiones concretas de la corrida real.
