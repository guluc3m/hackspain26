...
#adr([Rung 5 en la nube: deepseek-v4.1-flash; revisión humana no bloqueante],
  ...
  evidencia: [DRILLS MEDIDOS (volcado en `video/data_drills.json`, T12): `rung5-provider-caido` PASS — el proveedor no responde tras 3 intentos y la página entra en cola de revisión con motivo en evidencia, el lote sigue; `backoff-429` PASS — Retry-After respetado, 3 llamadas exactas y delays aplicados según cabecera; `crash-reanudacion` y `ledger-corrupto` PASS (4/4 drills). Cola de revisión en la UI con imagen de página junto a cada lectura candidata y desacuerdo resaltado (T5/T7); números de la cola de revisión de la corrida real: PENDIENTE-MEDICIÓN(T14). Decisión registrada en `docs/decisiones/DECISIONS.md` y AGENTS §3 (escalera de extracción).],
)
