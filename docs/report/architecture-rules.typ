...
El motor consume los campos que entrega el _parser_ — cada uno con _todos_ sus
candidatos — y los datos maestros, y produce la decisión: `PAGAR`, `NO_PAGAR` o
`ESCALAR`. Es puro y determinista: mismos campos + misma configuración + mismo
...
Cada regla vive en código y tiene un _código estable_ asociado, que es lo que
aparece en los _logs_ y en la salida, nunca cadenas de texto libres. Añadir una
regla no toca el bloque de extracción, y añadir un tipo de ficha no toca el
motor.
...
#figure(
  table(
    ...
    [`IBAN_MATCHES_MASTER`], [El IBAN de la factura coincide con el del maestro para ese NIF.],
    [`ORDER_BELONGS_TO_SUPPLIER`], [El pedido existe en el ERP, pertenece a ese proveedor y el importe de la factura cuadra con el suyo.],
...
== Agregación de veredictos

La frontera entre `NO_PAGAR` y `ESCALAR` es configuración, no código: un
`UNKNOWN` es siempre duda razonable (escala), mientras que un `FAIL` resuelve
al resultado configurado para esa regla — `NO_PAGAR` (negativo definitivo, valor
por defecto) o `ESCALAR` (un humano debe revisarlo). Una regla rota nunca puede
resolver `PAGAR`, y la carga de la configuración rechaza cualquier YAML que lo
intente. Ante duda razonable, escalar antes que pagar.

+ Si algún `FAIL` resuelve `NO_PAGAR` ⇒ `NO_PAGAR`.
...
+ Puntuación de cada candidato = confianza × peso del extractor. Cada campo
  tiene un _threshold_ de puntuación que hay que superar para continuar.
+ Gana la puntuación máxima. Empate con valores iguales: indiferente. Empate
  con valores distintos: se desempata por un ranking de extractores
  configurable por campo.

El proceso queda auditado candidato a candidato: cuál se eligió y por qué, y
qué pasó con el resto (rechazado por formato, bajo el umbral, desfavorecido en
el desempate). La elección se registra en la evaluación de la regla. Si no
...
Los _thresholds_ y la selección de candidatos viven en un YAML versionado cuyo
...
Los maestros (`proveedores.csv`, `pedidos.csv`) se cargan al arrancar: NIF con
su IBAN, y pedidos del ERP con proveedor, importe, estado y pago. Su `sha256`
queda sellado en cada decisión, de modo que se sabe qué maestro estaba activo
al decidir; el cruce contra el ERP se hace a través de una interfaz
intercambiable, sin acoplar el motor a ningún ERP concreto.
