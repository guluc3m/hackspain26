...
from filemaid.types import classify_unknown_reason
...
CREATE TABLE IF NOT EXISTS decisions (
  ...
  parser_ms INTEGER NOT NULL DEFAULT 0,
  evaluation_ms INTEGER NOT NULL DEFAULT 0,
  total_ms INTEGER NOT NULL DEFAULT 0,
  timings TEXT NOT NULL DEFAULT '{}', -- JSON con latencias detalladas
  timestamp REAL NOT NULL,
...
class Store:
    ...
    def _migrate(self) -> None:
        ...
        decision_cols = {r["name"] for r in self.conn.execute("PRAGMA table_info(decisions)")}
        for col, col_type, default_val in [
            ("extraction_ms", "INTEGER", "0"),
            ("parser_ms", "INTEGER", "0"),
            ("evaluation_ms", "INTEGER", "0"),
            ("total_ms", "INTEGER", "0"),
            ("timings", "TEXT", "'{}'"),
        ]:
            if col not in decision_cols:
                self.conn.execute(
                    f"ALTER TABLE decisions ADD COLUMN {col} {col_type} NOT NULL DEFAULT {default_val}"
                )
    ...
    def decision_rows_for_run(self, run_id: str) -> list[sqlite3.Row]:
        return self.conn.execute(
            """SELECT d.invoice_id, d.run_id, d.result, d.config_snapshot, d.timestamp,
                      d.extraction_ms, d.parser_ms, d.evaluation_ms, d.total_ms, d.timings,
                      i.file_id, i.sha256
    ...
    def save_decision(
        ...
        self.conn.execute(
            """INSERT OR REPLACE INTO decisions
               (invoice_id, run_id, result, config_snapshot, extraction_ms, parser_ms, evaluation_ms, total_ms, timings, timestamp)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch('now'))""",
            (
                invoice_id,
                run_id,
